...
export default class Redlock extends EventEmitter {
  ...
  private _hash (value: string): string {
    return createHash('sha1').update(value)
      .digest('hex')
  }
  ...
  public async acquire (
    ...
    if (Math.floor(duration) !== duration)
      throw new Error('Duration must be an integer value in milliseconds. ' + JSON.stringify(resources))
    ...
    while (true) {
      ...
      if (attempts.length < maxAttempts)
        ...
        throw new ExecutionError(
          'The operation was unable to achieve a quorum during its retry window.' + JSON.stringify(keys),
          attempts
        )
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (typeof shaResult !== 'number')
          throw new Error(
            `Unexpected result of type ${typeof shaResult} returned from redis.` + JSON.stringify(keys)
          )
        ...
        if (typeof rawResult !== 'number')
          throw new Error(
            `Unexpected result of type ${typeof rawResult} returned from redis.` + JSON.stringify(keys)
          )
      ...
      if (!(error instanceof Error))
        throw new Error(
          `Unexpected type ${typeof error} thrown with value: ${error} ` + JSON.stringify(keys)
        )
  ...
  public async using<T> (
    ...
    if (typeof routine !== 'function')
      throw new Error('INVARIANT: routine is not a function. ' + JSON.stringify(resources))

    if (settings.automaticExtensionThreshold > duration - 100)
      throw new Error(
        'A lock `duration` must be at least 100ms greater than the `automaticExtensionThreshold` setting. ' + JSON.stringify(resources)
      )
    ...
    async function extend (): Promise<void> {
      ...
      try {
        ...
        if (!(error instanceof Error))
          throw new Error(`Unexpected thrown ${typeof error}: ${error}. ${JSON.stringify(resources)}`)
