...
export class EventsSenderService {
  async init () {
    try {
      ...
      if (cronPattern == null)
        throw new Error('Error: missing cronpattern for event sender service')

      const sysLockName = env.EVENTS_SENDER_DOWNLOADER_LOCK_NAME
      const lockName = `${sysLockName}-${Component.UNIVERSAL_REPORTING}`
  ...
  constructor (@InjectQueue(QUEUE_NAMES.LocalEventsURQueue) private eventsQueue: Queue,
    ...
    this.localEventsServiceClient.setComponent(Component.EVENT_SENDER_UR)
