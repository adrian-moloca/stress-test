...
export class EventsSenderService {
  async init () {
    try {
      ...
      const lockName = `${sysLockName}-${Component.ROLES}`
  ...
  constructor (@InjectQueue(QUEUE_NAMES.LocalEventsRolesQueue) private eventsQueue: Queue,
    ...
    this.localEventsServiceClient.setComponent(Component.EVENT_SENDER_ROLES)
