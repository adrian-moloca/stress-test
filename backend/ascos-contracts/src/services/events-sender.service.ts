...
export class EventsSenderService {
  async init () {
    try {
      ...
      const lockName = `${sysLockName}-${Component.CONTRACTS}`
  ...
  constructor (@InjectQueue(QUEUE_NAMES.LocalEventsContractsQueue) private eventsQueue: Queue,
    ...
    this.localEventsServiceClient.setComponent(Component.EVENT_SENDER_CONTRACTS)
