...
export class EventsSenderService {
  async init () {
    try {
      ...
      const lockName = `${sysLockName}-${Component.PATIENT_ANAGRAPHICS}`
  ...
  constructor (
    @InjectQueue(QUEUE_NAMES.LocalEventsPatientAnagraphicsQueue) private eventsQueue: Queue,
    ...
    this.localEventsServiceClient.setComponent(Component.EVENT_SENDER_PATIENT_ANAGRAPHIC)
