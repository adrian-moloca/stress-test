...
export class EventsSenderService {
  async init () {
    try {
      ...
      const lockName = `${sysLockName}-${Component.OR_MANAGEMENT}`
  ...
  constructor (@InjectQueue(QUEUE_NAMES.LocalEventsORMangementQueue) private eventsQueue: Queue,
    ...
    this.localEventsServiceClient.setComponent(Component.EVENT_SENDER_ORMANAGEMENT)
