...
export class EventsSenderService {
  async init () {
    try {
      ...
      const lockName = `${sysLockName}-${Component.SYSTEM_CONFIGURATION}`
  ...
  constructor (@InjectQueue(QUEUE_NAMES.LocalEventsSystemConfigQueue) private eventsQueue: Queue,
    ...
    this.localEventsServiceClient.setComponent(Component.EVENT_SENDER_SYSTEM_CONFIG)
