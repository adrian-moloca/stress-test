...
export class EventsSenderService {
  async init () {
    try {
      ...
      const lockName = `${sysLockName}-${Component.USERS}`
  ...
  constructor (@InjectQueue(QUEUE_NAMES.LocalEventsUserQueue) private eventsQueue: Queue,
    ...
    this.localEventsServiceClient.setComponent(Component.EVENT_SENDER_USER)
